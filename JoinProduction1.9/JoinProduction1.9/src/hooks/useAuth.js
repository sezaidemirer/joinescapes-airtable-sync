...
export const useAuth = () => {
  ...
  const signUp = async (email, password, userRole = 'user', userData = {}) => {
    try {
      ...
      // Editör profili oluştur (eğer editör/yazar/supervisor ise) - EMAIL DOĞRULANMIŞ OLSUN OLMASIN
      if (data?.user && (userRole === 'editor' || userRole === 'yazar' || userRole === 'supervisor')) {
          try {
            ...
            if (profileError) {
              console.error('❌ Editör profili oluşturulamadı:', profileError)
            } else {
              console.log('✅ Editör profili oluşturuldu:', data.user.id)
            }
          } catch (error) {
            console.error('❌ Editör profili oluşturma hatası:', error)
          }
      }
