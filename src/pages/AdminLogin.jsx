...
const AdminLogin = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && !loading && user && user.id) {
      ...
      // Yazar onay kontrolü (test@test.com hariç)
      if (userRole === 'yazar' && user.email !== 'test@test.com') {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        // Kayıt ol
        console.log('🔍 Kayıt işlemi:', { email, selectedRole, firstName, lastName, birthDate })
        console.log('🔍 State değerleri:', { firstName, lastName, birthDate })
        
        // Editör bilgileri için userData oluştur
        const userData = {
          firstName: firstName || '',
          lastName: lastName || '',
          birthDate: birthDate || ''
        ...
        const { data, error } = await signUp(email, password, selectedRole, userData)
        ...
        if (error) {
          ...
          // Yazar için özel mesaj
          if (selectedRole === 'yazar') {
