...
const WriterProfile = () => {
  ...
  // Slug'dan name'e çevir (ters işlem) - Türkçe karakter desteği ile
  const slugToName = (slug) => {
    // Önce slug'ı düzeltilmiş isimlerle eşleştir
    const nameMapping = {
      'murat-arkin': 'Murat Arkın',
      'aleyna-gemi': 'Aleyna Gemi',
      'aleyna-rey-gemi': 'Aleyna Rey Gemi',
      'beste-akkor': 'Beste Akkor',
      'saykoarmes22-de': 'saykoarmes22 de'
    };
    
    // Eğer mapping'de varsa direkt kullan
    if (nameMapping[slug]) {
      console.log('🔍 DEBUG - slugToName mapping bulundu:', slug, '→', nameMapping[slug]);
      return nameMapping[slug];
    }
    
    // Yoksa normal dönüşüm yap
    const result = slug
      .replace(/-/g, ' ')
      .split(' ')
    ...
    console.log('🔍 DEBUG - slugToName normal dönüşüm:', slug, 'output:', result);
    return result;
  };
  ...
  useEffect(() => {
    const fetchWriter = async () => {
      ...
      try {
        console.log('🔍 Yazar aranıyor, slug:', slug);

        // ÖNCELİK 1: Slug kolonundan direkt ara (en güvenli)
        let { data, error } = await supabase
        ...
        console.log('🔍 Slug ile arama sonucu:', { data, error });

        // Slug ile bulunamazsa, eski yöntemle dene (backwards compatibility)
        if (error && error.code === 'PGRST116') {
          console.log('🔍 Slug ile bulunamadı, name ile deneniyor...');
          ...
          if (error && error.code === 'PGRST116') {
            console.log('🔍 Name ile bulunamadı, kısmi arama yapılıyor...');
        ...
        // Eğer writer_profiles'da bulunamazsa, posts tablosundan author_name ile dene
        if (!data && error) {
          console.log('🔍 Writer_profiles\'da bulunamadı, posts tablosundan deneniyor:', searchName);
          
          // Posts tablosundan author_name ile ara
          ...
          if (postsData && postsData.length > 0) {
            const authorName = postsData[0].author_name;
            console.log('✅ Posts tablosunda yazar bulundu:', authorName);
            
            // Bu author_name ile fake bir writer objesi oluştur
            data = {
              id: 'temp-' + Date.now(),
              name: authorName,
              title: 'Yazar',
              bio: 'Bu yazar hakkında detaylı bilgi henüz eklenmemiş.',
              location: 'Türkiye',
        ...
        if (error) {
          ...
          const safeParseSpecialties = (value) => {
            try {
              if (!value) return [];
              ...
              console.warn('⚠️ Specialties parse hatası:', e);
              return [];
            }
          };

          const safeParseSocialMedia = (value) => {
            try {
              if (!value) return {};
              ...
              console.warn('⚠️ Social media parse hatası:', e);
              return {};
            }
          };
          ...
          const formattedWriter = {
            ...
            image: data.profile_image || null, // Fallback fotoğrafı kaldır, mavi avatar gösterilecek
            ...
            posts: posts.length,
            specialties: safeParseSpecialties(data.specialties),
            socialMedia: safeParseSocialMedia(data.social_media),
            recentPosts: posts
          };
          ...
          <div className="flex flex-col md:flex-row items-center md:items-start space-y-6 md:space-y-0 md:space-x-8">
            <div className="relative">
              {writer.image ? (
                ...
                <div className="w-32 h-32 md:w-40 md:h-40 bg-blue-500 rounded-full border-4 border-white/20 shadow-xl flex items-center justify-center">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <div className="flex flex-wrap justify-center md:justify-start items-center gap-4 sm:gap-6 text-sm">
              ...
              {writer.recentPosts && writer.recentPosts.length > 0 ? (
                <div className="space-y-6">
                  {writer.recentPosts.map((post, index) => (
                    <div key={index} className="flex flex-col sm:flex-row space-y-3 sm:space-y-0 sm:space-x-4 p-4 rounded-lg hover:bg-gray-50 transition-colors">
                      <img 
                        src={post.image} 
                        alt={post.title}
