-- Writer profiles'daki kullanıcıların rollerini kontrol et

SELECT 
    wp.id as profile_id,
    wp.name as yazar_adi,
    wp.email as profile_email,
    wp.is_featured as one_cikan,
    au.email as auth_email,
    au.raw_user_meta_data->>'user_role' as user_role,
    au.raw_user_meta_data->>'editor_approved' as editor_approved,
    wp.created_at as kayit_tarihi
FROM writer_profiles wp
LEFT JOIN auth.users au ON wp.user_id = au.id
ORDER BY wp.created_at DESC;

-- Normal kullanıcıları temizle (eğer yanlışlıkla eklenmişse)
-- DELETE FROM writer_profiles
-- WHERE user_id IN (
--     SELECT id FROM auth.users 
--     WHERE raw_user_meta_data->>'user_role' = 'user'
--        OR raw_user_meta_data->>'user_role' IS NULL
-- );

